...
### 根因
进球描述数组包含"角球开出…""任意球直接破门…""任意球绕过人墙…"。足球规则中角球和直接任意球破门**不可能越位**，VAR 检查越位在此类进球上出现违反现实。

### 修复
从主队和客队进球描述数组中删除所有角球和直接任意球相关描述（共 5 条），替换为其他开放式进球场景。

### 测试用例
```
用例12.1: grep "角球" src/pages/MatchPage.tsx → 进球描述数组中无匹配
用例12.2: grep "任意球直接" src/pages/MatchPage.tsx → 进球描述数组中无匹配
用例12.3: 所有进球场景均可被 VAR 越位检查（符合足球规则）
```
