...
## Bug #10: 非玩家小组不逐轮模拟，积分榜一成不变

**日期:** 2026-06-28  
**严重度:** 🔴 游戏逻辑  
**文件:** `src/engine/tournament.ts`

### 根因
`generateTournament` 中非玩家小组在初始化时一次性预模拟全部 6 场比赛（3 轮 × 2 场），之后玩家比赛时其他小组积分榜完全不变。玩家小组的 AI 比赛却每轮模拟，造成不一致。

### 修复
1. `generateTournament`: 删除预模拟循环，所有小组只初始化积分表
2. `recordGroupMatchResult`: 玩家比赛后，按当前轮次（0/1/2）同步模拟**所有 12 个小组**的对应轮次比赛（每轮每小组 2 场）
3. 轮次对阵: 第1轮=[0v1, 2v3], 第2轮=[0v2, 1v3], 第3轮=[0v3, 1v2]

### 测试用例
```
用例10.1: 玩家第1场小组赛后 → 所有12组积分榜均新增一轮数据
用例10.2: 玩家第2场小组赛后 → 所有组 played 数正确推进
用例10.3: 小组赛结束 → 所有组均完成3轮6场比赛
用例10.4: 查看小组积分榜 → 非玩家组积分随轮次变化
```
...
### 根因
`event_result` 阶段进球事件只显示球员名（如"梅西"），没有展示**怎么进球的**（如"禁区外远射直挂死角"）。完整描述虽然在 event 对象中，但 UI 未渲染。

### 修复
`event_result` 渲染中，对 `type === 'goal'` 事件增加 description 展示：球员名（金色加粗）+ 进球描述（灰色小字）。

### 测试用例
```
用例11.1: 进球后 event_result 阶段 → 显示球员名 + 进球方式描述
用例11.2: 乌龙球 → 显示乌龙描述
用例11.3: 非进球事件 → 保持原有展示
```
...
## Bug #12: VAR 越位检查不应触发于角球/任意球/点球破门
...
### 根因
VAR 越位检查对所有进球一视同仁（12% 概率触发）。但足球规则中，**角球、直接任意球、点球**破门不可能越位，VAR 不应在此类进球上检查越位。

### 修复（正确方案）
**不删除进球描述**（保留角球/任意球破门等精彩描述），而是在 VAR 越位触发条件中增加关键词过滤：
```ts
const noOffsideKeywords = ['角球', '任意球直接', '点球']
const canBeOffside = !noOffsideKeywords.some(kw => goalDesc.includes(kw))
if (Math.random() < 0.12 && canBeOffside) { /* 触发 VAR */ }
```
两处 VAR 路径均已添加此检查（goal_opportunity 路径 + 预生成 goal 路径）。

### 测试用例
```
用例12.1: 角球破门 → 不触发 VAR 越位检查
用例12.2: 直接任意球破门 → 不触发 VAR 越位检查
用例12.3: 点球破门 → 不触发 VAR 越位检查（如适用）
用例12.4: 远射/头球/单刀等开放式进球 → 正常触发 VAR（12%）
用例12.5: 角球/任意球进球描述仍存在于数组中（不误删）
```
